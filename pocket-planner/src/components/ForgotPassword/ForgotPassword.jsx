...
const ForgotPassword = () => {
    ...
    const second_message = "We have sent a code to your email with a verification number!"
    const third_message = "Set your new password to login into your account!"
    const fourth_message = "Your password has been successfully reset, login to access your account!"
  ...
  return (
    <div>
      ...
      {pageNumberValue == 0 && <InputBox icon={emailIcon} placeholder="Ex: austin@example.com" value={emailInputValue} onChange={handleEmailInputChange} type="email"/>}
      {pageNumberValue == 1 && <InputBox placeholder="Ex: 123456" value={verificationInputValue} onChange={handleVerificaitonInputChange} type="text"/>}
      {pageNumberValue == 2 && <InputBox icon={passwordIcon} placeholder="password" value={passwordInputValue} onChange={handlePasswordInputChange} type="password"/>}
      </div>
      <div className='button-container'>
      <GeneralButton label={buttonTitle} onClick={submitHander} paddingTop="20px" paddingBottom="20px"/>
      </div>
    </div>
  )
}
